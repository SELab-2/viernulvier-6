...
impl<'a> ProductionRepo<'a> {
    ...
    pub async fn insert_on(
        ...
        production: ProductionCreate,
        translations: Vec<ProductionTranslationData>,
    ) -> Result<Production, DatabaseError> {
        let production = sqlx::query_as::<_, Production>(
            "INSERT INTO productions (source_id, slug, video_1, video_2, eticket_info, uitdatabank_theme, uitdatabank_type)
             VALUES ($1, $2, $3, $4, $5, $6, $7)
             ON CONFLICT (source_id) DO UPDATE SET
        ...
        Self::upsert_translations_on(conn, production.id, &translations).await?;
    ...
    pub async fn update_on(
        ...
        let production = production.update_all_fields(&mut *conn).await?;
        Self::upsert_translations_on(conn, production.id, &translations).await?;
        Ok(production)
    }
    ...
    async fn upsert_translations_on(
        conn: &mut sqlx::PgConnection,
        production_id: Uuid,
