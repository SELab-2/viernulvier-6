...
use db::models::import_row::{ImportRowStatus, RawCell};
use db::models::import_session::{ImportMapping, ImportSessionStatus};
use db::repos::import::{CreateSession, NewImportRow};
...
pub struct RowsQuery {
    ...
    #[serde(default = "default_limit")]
    pub limit: u32,
    pub status: Option<db::models::import_row::ImportRowStatus>,
}
...
pub async fn revert_row(
    ...
    let revertible = matches!(
        row.status,
        db::models::import_row::ImportRowStatus::Created
            | db::models::import_row::ImportRowStatus::Updated
    );
...
async fn rollback_committed_rows(
    state: &AppState,
    session: &db::models::import_session::ImportSession,
    strict: RollbackMode,
) -> Result<(), AppError> {
    let adapter = state
        .import_registry
        ...
        if let Some(entity_id) = row.target_entity_id {
            ...
            match adapter.revert_row(entity_id, &state.db, &mut tx).await {
                ...
                Err(e) => {
                    ...
                    warnings.push(db::models::import_row::ImportWarning {
                        field: None,
