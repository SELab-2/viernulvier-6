...
use db::Database;
use db::models::entity_type::EntityType;
use db::models::facet::Facet;
use db::models::user::{UserCreate, UserRole};
...
pub struct AppState {
    ...
    pub revoked: db::revocation::RevokedUsers,
}
...
pub async fn start_app(config: AppConfig) -> Result<(), AppError> {
    ...
    let revoked = db::revocation::RevokedUsers::new(std::time::Duration::from_secs(
...
mod tests {
    ...
    async fn bootstrap_admin_creates_admin_when_none_exists(db: PgPool) {
        ...
        assert_eq!(user.role, db::models::user::UserRole::Admin);
    }
