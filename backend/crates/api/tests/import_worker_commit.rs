...
async fn commit_writes_productions_from_legacy_csv(pool: PgPool) {
    ...
    db.imports()
        .update_status(
            session_id,
            db::models::import_session::ImportSessionStatus::DryRunPending,
            None,
        )
        .await
    ...
    db.imports()
        .update_status(
            session_id,
            db::models::import_session::ImportSessionStatus::Committing,
            None,
        )
        .await
    ...
    assert_eq!(
        session.status,
        db::models::import_session::ImportSessionStatus::Committed,
...
async fn commit_marks_session_failed_when_one_row_errors(pool: PgPool) {
    ...
    assert_eq!(
        session.status,
        db::models::import_session::ImportSessionStatus::Failed
    );
    assert!(
        session.committed_at.is_some(),
